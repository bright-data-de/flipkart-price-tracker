"""
Flipkart Price Tracker - Bright Data Web Scraper API
----------------------------------------------------
Track prices, availability, and product data from Flipkart.

Docs  : https://docs.brightdata.de/scraping-automation/web-scraper-api/overview
Info  : https://brightdata.de/products/insights/price-tracker/flipkart
"""
...
# Search for "Flipkart" to find the matching Web Scraper (format: gd_xxxxxxxxxx).
DATASET_ID = os.getenv("BRIGHTDATA_DATASET_ID", "")
if not DATASET_ID:
    raise EnvironmentError(
        "BRIGHTDATA_DATASET_ID is not set.\n"
        "1. Navigate to: https://brightdata.de/cp/scrapers\n"
        "2. Search for 'Flipkart' and copy the Web Scraper ID.\n"
...
def track_prices(urls: list[str], **kwargs) -> list[dict]:
    """
    Collect price data for a list of Flipkart product URLs.
...
def discover_by_keyword(keyword: str, limit: int = 50, **kwargs) -> list[dict]:
    """
    Discover Flipkart products by keyword search.
...
def discover_by_category(category_url: str, limit: int = 100, **kwargs) -> list[dict]:
    """
    Discover all products from a Flipkart category page.

    Example:
        results = discover_by_category("https://flipkart.com/s?k=headphones", limit=200)
    """
...
if __name__ == "__main__":
    ...
    output_file = "flipkart_prices.json"
    ...
    # cat_results = discover_by_category("https://flipkart.com/category/sample", limit=50)
